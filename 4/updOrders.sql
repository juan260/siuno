...
set
  ...
  totalamount = (netamount + NEW.price - OLD.price) * (1+(tax/100))
where
...
set
  ...
  totalamount = (netamount + NEW.price) * (1+(tax/100))
where
  ORD.orderid = NEW.orderid
  and ORD.status IS NULL;
RETURN NULL;
...
update
  orders as ORD
set
  netamount = ORD.netamount - OLD.price + NEW.price, -- Restamos el precio
  totalamount = (netamount - OLD.price + NEW.price) * (1+(tax/100))
where
...
AFTER
UPDATE
  ON orderdetail FOR EACH ROW WHEN (OLD.price < NEW.price) EXECUTE PROCEDURE updAddOrders();

-- Trigger para cuando se actualice orderdetail quitando cantidades
CREATE TRIGGER updSubOrdersTrig
AFTER
UPDATE
  ON orderdetail FOR EACH ROW WHEN (OLD.price > NEW.price) EXECUTE PROCEDURE updSubtractOrders();
